...
class NuSceneOcc(NuScenesDataset):    
    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, **eval_kwargs):
        ...
        if len(inst_preds) > 0:
            ...
            results = main_iou(occ_preds, occ_gts)
            return results
        else:
            # return main_rayiou(occ_preds, occ_gts, lidar_origins, occ_class_names=occ_class_names)
            return main_iou(occ_preds, occ_gts)

    def evaluate_definedGT(self, occ_results, val_loader, runner=None, show_dir=None, vis=False, vis_dir=None, **eval_kwargs):
