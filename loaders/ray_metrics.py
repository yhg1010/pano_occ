...
def main_iou(sem_pred_list, sem_gt_list, ignore_masks=None):
    eval_miou = Metric_mIoU()

    for sem_pred, sem_gt in tqdm(zip(sem_pred_list, sem_gt_list), ncols=50):
        valid_mask = sem_gt != 17
        sem_pred = sem_pred[valid_mask]
        sem_gt = sem_gt[valid_mask]
        eval_miou.add_batch(sem_pred, sem_gt)
