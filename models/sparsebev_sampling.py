...
def sampling_4d(sample_points, mlvl_feats, scale_weights, lidar2img, image_h, image_w, eps=1e-5):
    B, Q, T, G, P, S, _ = sample_points.shape  # [B, Q, T, G, P, 4]
    N = 6
    sample_points = sample_points.reshape(B, Q, T, G * P * S, 3)
    ...
    lidar2img = lidar2img.expand(B, T*N, Q, G * P * S, 4, 4)
    lidar2img = lidar2img.reshape(B, T, N, Q, G*P*S, 4, 4)

    # expand the points
    ones = torch.ones_like(sample_points[..., :1])
    sample_points = torch.cat([sample_points, ones], dim=-1)  # [B, Q, GPS, 4]
    sample_points = sample_points[:, :, None, ..., None]     # [B, Q, T, GPS, 4]
    sample_points = sample_points.expand(B, Q, N, T, G * P * S, 4, 1)
    sample_points = sample_points.transpose(1, 3)   # [B, T, N, Q, GPS, 4, 1]

    # project 3d sampling points to image
    sample_points_cam = torch.matmul(lidar2img, sample_points).squeeze(-1)  # [B, T, N, Q, GPS, 4]
    # homo coord -> pixel coord
    homo = sample_points_cam[..., 2:3]
    homo_nonzero = torch.maximum(homo, torch.zeros_like(homo) + eps)
    sample_points_cam = sample_points_cam[..., 0:2] / homo_nonzero  # [B, T, N, Q, GPS, 2]
    ...
    valid_mask = valid_mask.permute(0, 1, 3, 4, 2)  # [B, T, Q, GPS, N]
    sample_points_cam = sample_points_cam.permute(0, 1, 3, 4, 2, 5)  # [B, T, Q, GPS, N, 2]
    ...
    i_point = torch.arange(G * P * S, dtype=torch.long, device=sample_points.device)
    i_batch = i_batch.view(B, 1, 1, 1, 1).expand(B, T, Q, G * P * S, 1)
    i_time = i_time.view(1, T, 1, 1, 1).expand(B, T, Q, G * P * S, 1)
    i_query = i_query.view(1, 1, Q, 1, 1).expand(B, T, Q, G * P * S, 1)
    i_point = i_point.view(1, 1, 1, G * P * S, 1).expand(B, T, Q, G * P * S, 1)
    i_view = torch.argmax(valid_mask, dim=-1)[..., None]  # [B, T, Q, GPS, 1]

    sample_points_cam = sample_points_cam[i_batch, i_time, i_query, i_point, i_view, :]  # [B, Q, GPS, 1, 2]
    valid_mask = valid_mask[i_batch, i_time, i_query, i_point, i_view]  # [B, Q, GPS, 1]
    sample_points_cam = torch.cat([sample_points_cam, i_view[..., None].float() / 5], dim=-1)
    sample_points_cam = sample_points_cam.reshape(B, T, Q, G, P, S, 1, 3)
    sample_points_cam = sample_points_cam.permute(0, 1, 3, 2, 4, 5, 6, 7)  # [B, T, G, Q, P, S, 1, 3]
    sample_points_cam = sample_points_cam.reshape(B*T*G, Q, P * S, 3)

    scale_weights = scale_weights.reshape(B, Q, G, T, P, S, -1)
    scale_weights = scale_weights.permute(0, 2, 3, 1, 4, 5, 6)
    scale_weights = scale_weights.reshape(B*G*T, Q, P * S, -1)

    final = msmv_sampling(mlvl_feats, sample_points_cam, scale_weights)
    C = final.shape[2]  # [BTG, Q, C, PS]
    voxel_feat = final.clone()
    final = final.reshape(B, T, G, Q, C, P, S)
    final = final.permute(0, 6, 4, 3, 2, 1, 5)
    # final = final.flatten(1, 2)  # [B, Q, G, FP, C]
    final = final.reshape(B, S*C, Q, G, T*P)  # [B, TSC, Q, G, P]
    voxel_feat = voxel_feat.reshape(B, T, G, Q, C, P, S)
    voxel_feat = voxel_feat[..., 2]
    voxel_feat = voxel_feat.permute(0, 3, 2, 1, 5, 4)
    voxel_feat = voxel_feat.flatten(3, 4)
    return final, voxel_feat
